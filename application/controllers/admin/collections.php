...
class Collections extends MY_Controller {
	...
	public function index() {
		...
		$content=$this->load->view('collections/index', $result,true);
	
		$this->template->write('content', $content,true);
		$this->template->write('title', t('collections_management'),true);
	...
	{
		$this->html_form_url=site_url().'/admin/collections';		
		...
		if ($this->form_validation->run() == TRUE)
		{
			...
			$options['url']=url_title($options['thumbnail'], 'dash', TRUE);
															
			if ($id==NULL)
			{
				$options['pid']=0;
				$db_result=$this->Collections_model->insert($options);
			}
			else
			{
				//update db
				$db_result=$this->Collections_model->update($id,$options);
			}
		...
		else //loading form the first time
		{
				...
				{
					//get menu from db
					$obj=$this->Collections_model->select_single($id);
		...
		//show form
		$content=$this->load->view('collections/edit',$obj,true);									
	...
	{			
		...
		else if ($this->input->post('submit')!='')
		{
			...
			{
				//confirm delete	
				$this->Collections_model->delete($item);
			}
	...
	private function _search() {
		...
		//records
		$rows=$this->Collections_model->search($per_page, $offset,$filter, $sort_by, $sort_order);

		//total records in the db
		$total = $this->Collections_model->search_count();
		...
		$base_url = site_url('admin/collections');
