...
<!--
Ths transform produces an HTML data appraisal of a DDI 1/2.x XML document
...
<xsl:stylesheet version="1.0" xmlns:ddi="http://www.icpsr.umich.edu/DDI" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="ddi:codeBook">
				...
				<div class="xsl-title"><xsl:call-template name="gettext"><xsl:with-param name="msg">Data Appraisal</xsl:with-param></xsl:call-template></div>
				<xsl:choose>
					<xsl:when test="ddi:stdyDscr//ddi:EstSmpErr | ddi:stdyDscr//ddi:dataAppr">
								<!--Data Appraisal -->
								<table >
	...
	<xsl:template match="ddi:stdyDscr//ddi:sumDscr">
		<xsl:if test="ddi:collDate/@event | ddi:collDate/@date">
			<tr valign="top">
				<td colspan="2">
                	<div class="xsl-subtitle">Data Collection Dates</div>
	...
	<xsl:template match="ddi:stdyDscr//ddi:collMode">
		<tr valign="top">
			<td colspan="2"><div class="xsl-subtitle">Data Collection Mode</div>
				<xsl:value-of select="."/>
			</td>
		</tr>
	</xsl:template>	
...
<!-- End Data Processing & Appraisal -->	
	...
	<xsl:template match="*" name="row" mode="row">
		...
		<xsl:variable name="label">
			<div class="xsl-caption"><xsl:call-template name="gettext"><xsl:with-param name="msg"><xsl:value-of select="$caption"/></xsl:with-param></xsl:call-template></div>
			<xsl:choose>
