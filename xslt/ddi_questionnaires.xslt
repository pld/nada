...
<xsl:stylesheet version="1.0" xmlns:ddi="http://www.icpsr.umich.edu/DDI" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="ddi:codeBook">
				...
				<div class="xsl-title"><xsl:call-template name="gettext"><xsl:with-param name="msg">Questionnaires</xsl:with-param></xsl:call-template></div>
                				
						<!--questionnaires -->
	                     <xsl:choose>
                            <xsl:when test="ddi:stdyDscr//ddi:resInstru">
                                <xsl:apply-templates select="ddi:stdyDscr//ddi:resInstru" mode="row">
                                    <xsl:with-param name="caption"><xsl:call-template name="gettext"><xsl:with-param name="msg">Overview</xsl:with-param></xsl:call-template></xsl:with-param>
...
<!-- End Data Processing & Appraisal -->	
	...
	<xsl:template match="*" name="row" mode="row">
		...
		<xsl:variable name="label">
			<div class="xsl-subtitle" ><xsl:value-of select="$caption"/></div>
			<xsl:choose>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<!-- 1-column -->
						<div class="xsl-subtitle" style="margin-top:10px;">
							<xsl:value-of select="$label"/>
						</div>
						<xsl:choose>
							<xsl:when test="normalize-space($text)">
